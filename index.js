...
app.get('/', (req, res) => {
  res.render('index', {
    ...
    req,
    res,
	  render: (path) => ejs.render(require('fs').readFileSync(__dirname + "/public/" + path).toString())
  })
});
app.get('/howcanyouhelp', (req,res) => {
	res.render('howyoucanhelp', {
    // data,
    req,
    res,
	  render: (path) =>{ 
console.log(path, require('fs').readFileSync(__dirname + "/public/" + path).toString())
	return ejs.render(require('fs').readFileSync(__dirname + "/public/" + path).toString())
  }
})
...
const server = app.listen(3000, () => {
  console.log('server started on port ::3000\n https://Global-Warming.neongamerbotqk.repl.co \n or \n http://localhost:3000');
});
; (() => {
  function close() {
...
passport.use(new GoogleStrategy({
  ...
  callbackURL: "https://Global-Warming.neongamerbotqk.repl.co/auth/google/callback"
},
